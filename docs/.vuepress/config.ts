...
import { default as custom, default as CustomConfig, defaultCustomConfig } from './custom'
...
const config = defineUserConfig({
  title: custom?.config.app?.title ?? 'VP Starter',
  description: custom?.config.app?.description ?? 'A VuePress2 Template for building documentation websites.',

  // extra tags in `<head>`
  head: head(custom?.config.app?.base ?? defaultBase).concat(custom?.config.app?.head ?? []),

  // set site base to default value
  base: custom?.config.app?.base ?? defaultBase,

  // set site lang to default value
  lang: custom?.config.app?.lang ?? 'zh-CN',

  port: custom?.config.app?.port ?? 3000,
  open: custom?.config.app?.open ?? false,

  public: custom?.config.app?.public ?? path.join(__dirname, 'public'),
  ...
  define: custom?.config.app?.define ?? {
  ...
  // configure markdown
  markdown: custom?.config.app?.markdown ?? {
    importCode: {}
  }
})
